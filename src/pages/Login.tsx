...
function Login() {
  ...
  // Get supervisor type from URL pathname
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      if (location.pathname.includes('inspector')) {
        ...
        // Para supervisores, solo verificamos que exista el email y PIN en la tabla
        ...
        // Si las credenciales son correctas, redirigimos sin autenticación
        navigate(isSupervisorDelegation ? '/supervisor/delegacion' : '/supervisor/centro');
      } else if (portal === 'empleado') {
        // First verify employee exists and is active
        ...
        // Create session using RLS bypass function
        ...
        // Store employee ID in localStorage for later use
        localStorage.setItem('employeeId', employeeData.id);

        navigate('/empleado');
      } else {
        // Para empresas, mantenemos la autenticación normal
  ...
  const handleRecoverySubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      // Send email with PIN using EmailJS
